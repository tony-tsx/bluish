...
class MemorySessionStore extends SessionStore<Session> {
...
it('adds session in request response cookie', async () => {
  ...
  await CoreTesting.runMiddleware(cookie({ secret: 'secret' }), context)
...
it('skip session middleware if session already definied', async () => {
  ...
  await CoreTesting.runMiddleware(
    session({ store, generateIdentifier }),
    context,
  )

...
import { ControllerMetadataArgs, onApp } from '../index.js';
import { isTrusty } from '../tools/isTrusty.js';
import { init } from '../tools/init.js';
import { ON_APP_LISTENERS } from '../tools/onApp.js';
...
export class App {
  ...
  private async _initializeController(controllerMetadataArgs: ControllerMetadataArgs) {
    ...
    controller.handlers = await Promise.all(
      metadataArgsStorage
        ...
        .map(async args => {
          ...
          for (const middleware of handler.middlewares) await middleware.onApp?.(this);
    ...
    for (const middleware of controller.middlewares) await middleware.onApp?.(this);
  ...
  public async initialize() {
    ...
    try {
...

      {
        ...
        for (const middleware of this.middlewares) await middleware.onApp?.(this);

        for (const listener of onApp[ON_APP_LISTENERS]) await listener(this);
      }
    } catch (error) {
...
export interface App extends Bluish.App {}
